...
class TwitterService:
    def __init__(self):
        try:
            ...
            self.retry_count = 0
            self.max_retries = 5  # Increased for reliability
            self.post_timeout = 15  # 15 seconds for posts
    ...
    def post_article(self, article):
        ...
        try:
            ...
            logging.error(f"Post article error: {e}")
            return False
    ...
    def _post_with_retries(self, text, article):
        ...
        for attempt in range(self.max_retries):
            try:
                ...
                logging.info(f"Attempt {attempt+1}/{self.max_retries} to post tweet: {text[:50]}...")
                ...
                if response and hasattr(response, 'data') and response.data:
                    ...
                    self._add_activity_log('post', f"Posted tweet ID {tweet_id}: {text[:50]}...")
                    logging.info(f"Successfully posted tweet ID {tweet_id}")
                    return True
                else:
                    logging.warning(f"Empty response or missing data from Twitter API")
                    
            except tweepy.TooManyRequests as e:
                logging.warning(f"Rate limited on attempt {attempt+1}: {str(e)}")
                ...
                self._add_activity_log('error', f"Authentication error: {str(e)}")
                return False  # Don't retry auth errors
                
            except tweepy.BadRequest as e:
                logging.error(f"Twitter bad request error: {str(e)}")
                self._add_activity_log('error', f"Bad request: {str(e)}")
                # Only retry if not a permanent error (like invalid text)
                if "Tweet text too long" in str(e):
                    # Try with shorter text next time
                    text = text[:250] + "..."
                    continue
                return False
                ...
                continue        logging.error(f"Failed to post tweet after {self.max_retries} attempts")
